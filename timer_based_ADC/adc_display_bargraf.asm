...
BARGRAF_DISPLAY:
		...
		andlw	b'11110000'	;and most significiant nibble
...
BIT_7_NOT:
		btfss   DISPLAY_TEMP1,6 ;if the '4' bit is set
		goto    BIT_6_NOT
		bsf     STATUS,C
		rlf     DISPLAY_RESULT_L
		rlf	DISPLAY_RESULT_H
...
BIT_6_NOT:
		...
		btfss   DISPLAY_TEMP1,5 ;if the '2' bit is set
		goto    BIT_5_NOT
		bsf     STATUS,C
		rlf     DISPLAY_RESULT_L
		rlf	DISPLAY_RESULT_H
...
BIT_5_NOT:
		...
		btfss   DISPLAY_TEMP1,4 ;if the '1' bit is set
		goto    BIT_4_NOT
		bsf     STATUS,C
		rlf     DISPLAY_RESULT_L
		rlf	DISPLAY_RESULT_H
BIT_4_NOT:
		...
		bcf	PORTA,RA3 ; more significant bar is off (output should drive 8x0.020mA NPN transistor)
		nop
		bcf	PORTA,RA0 ; less significant bar is off (output should drive 8x0.020mA NPN transistor)
		nop
		bcf 	PORTA,RA4 ; discharge delay capacitor. 

		movfw 	DISPLAY_RESULT_L	; take the less significant result
		movwf 	PORTB			; out it to PORTB
		bsf	PORTA,RA0		; make current flow through LEDs
...
INTENSITY_DELAY1:
		...
		bcf	PORTA,RA0		; stop current flowing through LEDs

		movfw	DISPLAY_RESULT_H	; take the more significant result
		movwf 	PORTB			; out it to PORTB
		bsf	PORTA,RA3		; make current flow through LEDs
		bank1				; switch to bank1
		bsf	TRISA,RA4		; tristate RA4 pin - capacitor starts charging
		bank0				; switch back to bank0
INTENSITY_DELAY2:
		btfss	PORTA,RA4		; check if delay capacitor got charged, skip if yes
		goto 	INTENSITY_DELAY2	; if no, just wait for it to happen.

		bcf	PORTA,RA3		; stop current flowing through leds
						; now since all is done right, you may 
		return
